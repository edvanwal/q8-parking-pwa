...
total++;
if (
  runTest('Pricing: Flat Rate Calculation', () => {
...
total++;
if (
  runTest('Pricing: Estimated cost formatting for zone sheet', () => {
...
total++;
if (
  runTest('Pricing: missing zone.price uses fallback for cost', () => {
...
total++;
if (
  runTest('Pricing: missing rates – cost from zone.price', () => {
...
total++;
if (
  runTest('Pricing: rate_numeric max consistent with zone.price', () => {
    const zone = {
      price: 3.0,
      rates: [{ rate_numeric: 2.5 }, { rate_numeric: 3.0 }, { rate_numeric: 1.5 }],
    };
    const maxRate = Math.max(
      ...zone.rates.map((r) => (r.rate_numeric != null ? r.rate_numeric : 0))
    );
...
total++;
if (
  runTest('State: Initialization Defaults', () => {
    State.load();
    const s = State.get;
...
total++;
if (
  runTest('Services: Modify Duration', () => {
    // Setup mock zone
    State.update({
...
total++;
if (
  runTest('Services: Add/Delete Plate', () => {
    State.update({ plates: [] });
