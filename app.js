...
Q8.App = (function () {
  ...
  function handleClick(e) {
    try {
      ...
      if (
        inpSearch &&
        document.activeElement === inpSearch &&
        !e.target.closest("#ui-idle-search") &&
        !e.target.closest(".map-search-float")
      ) {
        inpSearch.blur();
      }
      ...
      switch (action) {
        ...
        case "select-quick-plate": {
          ...
          if (!qId)
            console.warn("[PLATE_SELECT] select-quick-plate: no data-id");
          ...
          if (returnTo === "modal-confirm-start") {
            ...
            if (!S.get.selectedZone)
              console.warn(
                "[PLATE_SELECT] Reopening zone sheet with no selectedZone",
              );
        ...
        case "open-overlay": {
          ...
          const context = {
            ...
            rates: (() => {
              try {
                return JSON.parse(target.getAttribute("data-rates") || "null");
              } catch (_) {
                return null;
              }
            })(),
          ...
          if (targetId === "modal-confirm-delete-plate") {
            ...
            if (modal)
              modal.setAttribute("data-delete-plate-id", plateId || "");
            const descEl = document.getElementById("confirm-delete-plate-desc");
            const plate = plateId
              ? S.get.plates.find((p) => p.id == plateId || p.text == plateId)
              : null;
            if (descEl)
              descEl.textContent = plate
                ? S.get.language === "nl"
                  ? `Weet u zeker dat u kenteken ${plate.text} wilt verwijderen? Dit kan niet ongedaan worden gemaakt.`
                  : `Are you sure you want to remove license plate ${plate.text}? This cannot be undone.`
                : S.get.language === "nl"
                  ? "Weet u zeker dat u dit kenteken wilt verwijderen?"
                  : "Are you sure you want to remove this license plate? This cannot be undone.";
          }
        ...
        case "close-overlay": {
          ...
          const returnTo = S.get.plateSelectorReturnTo;
          if (
            current === "sheet-plate-selector" &&
            returnTo === "modal-confirm-start"
          ) {
        ...
        case "mod-duration":
          ...
          if (isNaN(delta))
            console.warn(
              "[DURATION] mod-duration: invalid data-delta",
              target.getAttribute("data-delta"),
            );
        ...
        case "mod-active-end": {
          ...
          if (!isNaN(endDelta) && Services.modifyActiveSessionEnd)
            Services.modifyActiveSessionEnd(endDelta);
          break;
        }
        ...
        case "start-session": {
          ...
          if (Services.validateStartParking) {
            ...
            if (!result.valid) {
              ...
              if (
                result.field === "plate" &&
                document.getElementById("details-plate")
              ) {
          ...
          if (zoneObj && Services.hasDayPass && Services.hasDayPass(zoneObj)) {
            const cost =
              Services.getDayPassCost && Services.getDayPassCost(zoneObj);
            const costStr =
              cost != null ? ` €${String(cost).replace(".", ",")}` : "";
            ...
            if (cancelBtn)
              cancelBtn.textContent =
                S.get.language === "nl" ? "ANNULEREN" : "CANCEL";
            if (okBtn)
              okBtn.textContent =
                S.get.language === "nl" ? "JA, STARTEN" : "YES, START";
        ...
        case "confirm-start-daypass": {
          ...
          const daypassNote = nl
            ? "Let op: dit is een zone met dagkaart."
            : "Note: this zone has a day pass.";
        ...
        case "edit-plate": {
          const plateId = target.getAttribute("data-id");
          const plate = S.get.plates.find(
            (p) => p.id == plateId || p.text == plateId,
          );
          if (plate) {
        ...
        case "save-edit-plate": {
          const modal = document.getElementById("modal-edit-plate");
          const editingId = modal
            ? modal.getAttribute("data-editing-id")
            : null;
          const inpPlate = document.getElementById("inp-edit-plate");
          const inpDesc = document.getElementById("inp-edit-plate-desc");
          if (editingId && inpPlate) {
            Services.updatePlate(
              editingId,
              inpPlate.value,
              inpDesc ? inpDesc.value : "",
            );
          }
          break;
        }
        ...
        case "confirm-delete-plate": {
          const modal = document.getElementById("modal-confirm-delete-plate");
          const plateId = modal
            ? modal.getAttribute("data-delete-plate-id")
            : null;
        ...
        case "submit-forgot-password": {
          ...
          if (Services.sendPasswordResetEmail) {
            Services.sendPasswordResetEmail(email)
              .then(() => {
                if (resultEl) {
                  resultEl.style.display = "block";
                  resultEl.textContent =
                    S.get.language === "nl"
                      ? "Resetlink verzonden! Check je e-mail."
        ...
        case "toggle-notif-setting": {
          ...
          if (key && S.get.notificationSettings) {
            ...
            if (Services.syncNotificationSettingsToFirestore)
              Services.syncNotificationSettingsToFirestore(next);
          }
          break;
        }
        ...
        case "toggle-favorite": {
          ...
          if (exists) {
            ...
            const maxOrder = favs.length
              ? Math.max(...favs.map((f) => f.order ?? 0))
              : -1;
        ...
        case "remove-favorite": {
          ...
          const favs = (S.get.favorites || []).filter(
            (f) =>
              !(
                f.zoneUid === zoneUid ||
                f.zoneId === zoneUid ||
        ...
        case "add-favorite-from-history": {
          ...
          const next = [
            ...favs,
            {
              zoneUid: zoneUid || zoneId,
              zoneId: zoneId || zoneUid,
              order: maxOrder + 1,
            },
          ];
          S.update({ favorites: next });
        ...
        case "edit-favorite-name": {
          ...
          const promptMsg =
            S.get.language === "nl"
              ? "Naam voor deze favoriet (bijv. Werk, Thuis):"
              : "Name for this favorite (e.g. Work, Home):";
          const newName = (prompt(promptMsg, current) || "").trim();
          if (newName === current) break;
          const next = favs.map((f) =>
            f === fav ? { ...f, name: newName || undefined } : f,
          );
          S.update({ favorites: next });
          if (S.saveFavorites) S.saveFavorites();
          if (UI.showToast)
            UI.showToast(
              S.get.language === "nl" ? "Naam bijgewerkt" : "Name updated",
              "success",
            );
          break;
        }
        ...
        case "set-dark-pref": {
          ...
          document.querySelectorAll(".dark-mode-opt").forEach((btn) => {
            btn.setAttribute(
              "aria-pressed",
              btn.getAttribute("data-pref") === (S.get.darkMode || "system")
                ? "true"
                : "false",
            );
          });
        ...
        case "dismiss-onboarding":
          try {
            localStorage.setItem("q8_onboarding_done", "1");
          } catch (e) {
            /* ignore */
          }
        ...
        case "quick-filter-daterange": {
          ...
          S.update({
            historyFilters: {
              ...
              dateRange: range,
              customStart:
                range === "custom" ? S.get.historyFilters.customStart : null,
              customEnd:
                range === "custom" ? S.get.historyFilters.customEnd : null,
            },
          });
          break;
        }
        ...
        case "export-history-csv":
          if (Q8.Utils && Q8.Utils.exportHistoryToCSV)
            Q8.Utils.exportHistoryToCSV(S.get);
          if (UI.showToast)
            UI.showToast(
              S.get.language === "nl" ? "CSV gedownload" : "CSV downloaded",
              "success",
            );
          break;

        case "export-history-print":
          if (Q8.Utils && Q8.Utils.exportHistoryToPrint)
            Q8.Utils.exportHistoryToPrint(S.get);
          if (UI.showToast)
            UI.showToast(
              S.get.language === "nl"
                ? "Printdialoog geopend"
                : "Print dialog opened",
              "success",
            );
          break;
        ...
        case "retry-load-zones":
          ...
          if (Services && Services.loadZones)
            Services.loadZones().catch(() => {});
          break;
        ...
        case "go-to-my-location": {
          ...
          let explained = false;
          try {
            explained = !!localStorage.getItem(locKey);
          } catch (e) {
            /* ignore */
          }
          if (explained) {
            Services.requestUserLocation().then((loc) => {
              if (!loc && UI.showToast)
                UI.showToast(
                  S.get.language === "nl"
                    ? "Locatie niet beschikbaar. Controleer toestemming of GPS."
                    : "Location not available. Check permission or GPS.",
                  "error",
                );
            });
          } else {
            ...
            const titleEl = document.getElementById(
              "location-explanation-title",
            );
            const textEl = document.getElementById("location-explanation-text");
            const btnEl = document.querySelector(
              '#modal-location-explanation [data-action="confirm-location-explanation"]',
            );
            if (titleEl) titleEl.textContent = nl ? "Locatie" : "Location";
            if (textEl)
              textEl.textContent = nl
                ? "We gebruiken je locatie om nabije parkeerzones te tonen."
                : "We use your location to show nearby parking zones.";
        ...
        case "confirm-location-explanation":
          try {
            localStorage.setItem("q8_location_explanation_shown", "1");
          } catch (e) {
            /* ignore */
          }
          S.update({ activeOverlay: null });
        ...
        case "toggle-filter-vehicle":
          ...
          S.update({
            historyFilters: { ...S.get.historyFilters, vehicles: newVehicles },
          });
  ...
  function initListeners() {
    ...
    document.body.addEventListener("change", (e) => {
      ...
      if (
        ...
        e.target.getAttribute("data-action") === "change-expiring-interval"
      ) {
        const val = parseInt(e.target.value, 10);
        if (!isNaN(val) && S.get.notificationSettings) {
          const next = {
            ...S.get.notificationSettings,
            expiringSoonMinutes: val,
          };
          S.update({ notificationSettings: next });
          if (S.saveNotifications) S.saveNotifications();
          if (Services.syncNotificationSettingsToFirestore)
    ...
    if (inpPlate && typeof Q8 !== "undefined" && Q8.Kenteken) {
      function updatePlateFormatFeedback() {
        ...
        if (v.valid) {
          ...
          errEl.textContent = v.errorMessage || "";
      ...
      function formatPlateForDisplay() {
        ...
        if (v.valid && v.display) {
          ...
          if (formatted && formatted !== inpPlate.value)
            inpPlate.value = formatted;
        }
      }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("offline", () => {
    ...
    if (Q8.UI && Q8.UI.showToast) {
      Q8.UI.showToast(
        S.get && S.get.language === "nl"
          ? "Geen verbinding. Je kunt nog wel je actieve parkeren zien."
          : "No connection. You can still see your active parking.",
        "error",
      );
    }
  });
