      ...
      if (closeEl) {
        ...
        if (overlayId && S && S.update) {
          ...
          if (current === 'sheet-plate-selector' && returnTo === 'modal-confirm-start') {
            ...
            S.update({
              activeOverlay: null,
              ...(current === 'sheet-plate-selector' && { plateSelectorReturnTo: null }),
            });
          }
      ...
      switch (action) {
        ...
        case 'set-search-mode': {
          ...
          document
            ...
            .forEach((b) => b.classList.toggle('active', b.getAttribute('data-mode') === mode));
          break;
        }
        ...
        case 'select-quick-plate': {
          ...
          if (returnTo === 'modal-confirm-start') {
            ...
            if (!S.get.selectedZone)
              console.warn('[PLATE_SELECT] Reopening zone sheet with no selectedZone');
        ...
        case 'start-session': {
          ...
          if (zoneObj && Services.hasDayPass && Services.hasDayPass(zoneObj)) {
            ...
            if (titleEl)
              titleEl.textContent =
                S.get.language === 'nl' ? 'Zone met dagkaart' : 'Zone with day pass';
            if (descEl)
              descEl.textContent =
                S.get.language === 'nl'
          ...
          if (Services.sendPasswordResetEmail) {
            Services.sendPasswordResetEmail(email)
              .then(() => {
                ...
                if (UI.showToast)
                  UI.showToast(
                    S.get.language === 'nl' ? 'Resetlink verzonden' : 'Reset link sent',
                    'success'
                  );
              })
              .catch((err) => {
                if (resultEl) {
                  ...
                  resultEl.textContent =
                    err.message ||
                    (S.get.language === 'nl' ? 'Fout bij verzenden.' : 'Failed to send.');
                  resultEl.style.color = 'var(--danger)';
                }
                if (UI.showToast)
                  UI.showToast(
        ...
        case 'remove-favorite': {
          ...
          if (UI.showToast)
            UI.showToast(
              S.get.language === 'nl' ? 'Verwijderd uit favorieten' : 'Removed from favorites',
              'success'
            );
          break;
        }
        case 'add-favorite-from-history': {
          ...
          const next = [
            ...favs,
            { zoneUid: zoneUid || zoneId, zoneId: zoneId || zoneUid, order: maxOrder + 1 },
          ];
          S.update({ favorites: next });
          if (S.saveFavorites) S.saveFavorites();
          if (UI.showToast)
            UI.showToast(
              S.get.language === 'nl' ? 'Toegevoegd aan favorieten' : 'Added to favorites',
              'success'
            );
          break;
        }
        case 'edit-favorite-name': {
          ...
          if (S.saveFavorites) S.saveFavorites();
          if (UI.showToast)
            UI.showToast(S.get.language === 'nl' ? 'Naam bijgewerkt' : 'Name updated', 'success');
          break;
        }
        ...
        case 'set-dark-pref': {
          ...
          document.querySelectorAll('.dark-mode-opt').forEach((btn) => {
            btn.setAttribute(
              'aria-pressed',
              btn.getAttribute('data-pref') === (S.get.darkMode || 'system') ? 'true' : 'false'
            );
          });
          break;
        }
        ...
        case 'set-gate-lang':
          S.update({
            installMode: { ...S.get.installMode, language: target.getAttribute('data-lang') },
          });
        ...
        case 'dismiss-install-gate': {
          ...
          try {
            if (visitCount != null)
              localStorage.setItem('q8_install_gate_dismissed_at', String(visitCount));
          } catch (e) {
            /* ignore */
          }
        ...
        case 'export-history-csv':
          ...
          if (UI.showToast)
            UI.showToast(S.get.language === 'nl' ? 'CSV gedownload' : 'CSV downloaded', 'success');
          break;
        ...
        case 'export-history-print':
          ...
          if (UI.showToast)
            UI.showToast(
              S.get.language === 'nl' ? 'Printdialoog geopend' : 'Print dialog opened',
              'success'
            );
          break;
        ...
        case 'register':
          ...
          if (!rEmail || !rPass) {
            if (UI.showToast)
              UI.showToast(
                S.get.language === 'nl' ? 'Vul alle velden in' : 'Please fill in all fields',
                'error'
              );
            return;
          }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    if (Q8.UI && Q8.UI.showToast) {
      Q8.UI.showToast(
        S.get && S.get.language === 'nl' ? 'App geïnstalleerd' : 'App installed successfully!',
        'success'
      );
    }
  });
  ...
  window.addEventListener('online', () => {
    ...
    if (Q8.UI && Q8.UI.showToast) {
      Q8.UI.showToast(
        S.get && S.get.language === 'nl' ? 'Verbinding hersteld' : 'Connection restored',
        'success'
      );
    }
  });
