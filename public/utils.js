...
Q8.Utils = (function() {
    ...
    return {
        ...
        getFilteredHistory: function(state) {
            ...
            if (f.vehicles && f.vehicles.length > 0) list = list.filter(h => f.vehicles.includes(h.plate));
            ...
            if (dateStart || dateEnd) {
                list = list.filter(h => {
                    ...
                    if (dateStart) { const s = new Date(dateStart); s.setHours(0, 0, 0, 0); if (itemDate < s) return false; }
                    if (dateEnd) { const e = new Date(dateEnd); e.setHours(0, 0, 0, 0); if (itemDate > e) return false; }
                    return true;
                });
            }
            return list;
        ...
        exportHistoryToCSV: function(state) {
            ...
            const rows = list.map(h => [h.date || '', h.start || '', h.end || '', h.zone || '', (h.plate || '').replace(/"/g, '""'), (h.street || '').replace(/"/g, '""'), (h.price || '').toString().replace('.', ',')].map(c => `"${c}"`).join(';'));
        ...
        exportHistoryToPrint: function(state) {
            ...
            w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${nl ? 'Parkeerhistorie' : 'Parking History'}</title><style>body{font-family:sans-serif;padding:20px;}.table{width:100%;border-collapse:collapse;}th,td{border:1px solid #ddd;padding:8px;text-align:left;}th{background:#1E4A99;color:white;}.tot{font-weight:bold;margin-top:12px;}</style></head><body><h1>${nl ? 'Parkeerhistorie' : 'Parking History'}</h1><p>${new Date().toLocaleDateString(nl ? 'nl-NL' : 'en-GB')}</p><table class="table"><thead><tr><th>${nl ? 'Datum' : 'Date'}</th><th>${nl ? 'Start' : 'Start'}</th><th>${nl ? 'Einde' : 'End'}</th><th>${nl ? 'Zone' : 'Zone'}</th><th>${nl ? 'Kenteken' : 'License'}</th><th>${nl ? 'Kosten' : 'Cost'}</th></tr></thead><tbody>${list.map(h => `<tr><td>${h.date || ''}</td><td>${h.start || ''}</td><td>${h.end || ''}</td><td>${h.zone || ''}</td><td>${h.plate || ''}</td><td>€ ${(h.price || '').toString().replace('.', ',')}</td></tr>`).join('')}</tbody></table><div class="tot">${nl ? 'Totaal' : 'Total'}: ${list.length} ${nl ? 'sessies' : 'sessions'}</div></body></html>`);
            w.document.close();
            w.print();
            w.close();
        }
