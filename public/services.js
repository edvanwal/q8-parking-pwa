...
Q8.Services = (function () {
  ...
  const messaging =
    typeof firebase !== 'undefined' && firebase.messaging ? firebase.messaging() : null;
  ...
  function syncUserPlatesToFirestore(plates) {
    ...
    const userPlates = (plates || S.get.plates || []).filter(
      (p) => !(S.get.adminPlates || []).some((a) => a.id === p.id)
    );
    return db
      .collection('users')
      .doc(uid)
    ...
    return db
      ...
      .then((snap) => {
        ...
        if (session.start && !isNaN(session.start.getTime())) {
          ...
          if (S.save) S.save();
          if (U && U.debug)
            U.debug('AUTH', 'Session restored from Firestore after localStorage cleared');
        }
      })
      .catch(() => {});
  }
  ...
  function initFCMAndSaveToken(uid) {
    ...
    const vapidKey =
      typeof firebaseConfig !== 'undefined' && firebaseConfig.messagingVapidKey
        ? firebaseConfig.messagingVapidKey
        : '';
    ...
    return Notification.requestPermission()
      ...
      .catch((err) => {
        if (U && U.debug) U.debug('FCM', 'Token failed', err);
      });
  }
    ...
    db.collection('users')
      ...
      .update({
        notificationSettings: {
          ...
          expiringSoonMinutes:
            typeof settings.expiringSoonMinutes === 'number' ? settings.expiringSoonMinutes : 10,
        },
        updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      })
      .catch(() => {});
  ...
  function initAuthListener() {
    ...
    auth.onAuthStateChanged((user) => {
      if (user) {
        ...
        if (S.get.notificationSettings)
          syncNotificationSettingsToFirestore(S.get.notificationSettings);
  ...
  function updateNearbyFacilities() {
    ...
    if (nearbyFacilitiesRef === 'zone' && selectedZone && zones && zones.length) {
      const zone = zones.find(
        (z) => z.uid === selectedZone || (z.id && String(z.id) === String(selectedZone))
      );
  ...
  function parseAvailableCapacityFromSpdp(text) {
    ...
    if (s.startsWith('{')) {
      try {
        ...
        const v =
          ...
          j.AvailableCapacity ??
          j.available ??
          (j.ParkingFacilityStatus && j.ParkingFacilityStatus.availableCapacity);
    ...
    const m =
      s.match(/<AvailableCapacity[^>]*>(\d+)<\/AvailableCapacity>/i) ||
      s.match(/availableCapacity["\s:]+(\d+)/i);
      ...
      .catch((err) => {
        const msg =
          err && err.message
            ? err.message
            : S.get.language === 'nl'
              ? 'Garages & P+R konden niet worden geladen.'
              : 'Could not load garages & P+R.';
  ...
  function loadChargingPoints(bounds) {
    ...
    const query =
      '[out:json][timeout:15];node["amenity"="charging_station"](' +
      south +
      ',' +
      west +
      ',' +
    ...
    return fetch(OVERPASS_URL, {
      ...
      .then(function (data) {
        ...
        elements.forEach(function (el) {
          if (el.type === 'node' && el.lat != null && el.lon != null) {
            ...
            if (tags['socket:schuko'] || tags['socket:schuko:output'] || tags['socket:domestic'])
              connectors.push('schuko');
      ...
      .catch(function (err) {
        const msg =
          S.get.language === 'nl'
            ? 'Laadpunten konden niet worden geladen.'
            : 'Could not load charging points.';
            ...
            if (zones.length > 0) {
              requestAnimationFrame(() => {
                ...
                if (S.get.userLocation)
                  findAndSelectZoneAtLocation(S.get.userLocation.lat, S.get.userLocation.lng);
  ...
  function tryOpenOverlay(id, contextData = null) {
    ...
    if ((id === 'modal-add-plate' || id === 'modal-edit-plate') && S.get.screen !== 'plates')
      return;
    ...
    if (id === 'modal-confirm-start' && Q8.UI && Q8.UI.populateConfirmStartModal) {
      ...
      const opts =
        contextData && contextData.daypassNote ? { daypassNote: contextData.daypassNote } : {};
  ...
  function validateStartParking() {
    ...
    if (!S.get.selectedZone) {
      return {
        valid: false,
        message: nl ? 'Selecteer eerst een parkeerzone.' : 'Select a parking zone first.',
        field: 'zone',
      };
    }
    if (S.get.activeOverlay !== 'sheet-zone') {
      return {
        valid: false,
        message: nl ? 'Open een zone om te parkeren.' : 'Open a zone to start parking.',
        field: 'zone',
      };
    }
    const zoneObj =
    ...
    if (ds.allowedTimeStart || ds.allowedTimeEnd) {
      ...
      if (startM != null && mins < startM) {
        return {
          valid: false,
          message: nl ? 'Parkeren nog niet toegestaan.' : 'Parking not yet allowed.',
          field: 'zone',
        };
      }
  ...
  function handleStartParking(options) {
    const skipOverlayCheck =
      options && (options.fromDayPassConfirm === true || options.fromConfirmStart === true);
    if (S.get.session) {
    ...
    if (!skipOverlayCheck && S.get.activeOverlay !== 'sheet-zone') {
      ...
      toast(
        S.get.language === 'nl' ? 'Open een zone om te parkeren' : 'Open a zone to start parking.',
        'error'
      );
      return;
    }
    ...
    if (ds.allowedTimeStart || ds.allowedTimeEnd) {
      ...
      if (startM != null && mins < startM) {
        toast(
          S.get.language === 'nl' ? 'Parkeren nog niet toegestaan.' : 'Parking not yet allowed.',
          'error'
        );
        return;
      }
    ...
    if (!selPlate || !plateText) {
      toast(
        S.get.language === 'nl' ? 'Selecteer een kenteken.' : 'Select a license plate.',
        'error'
      );
      return;
    }
    ...
    addNotification(
      'sessionStarted',
      S.get.language === 'nl' ? 'Parkeersessie gestart' : 'Parking session started',
      `${displayId} · ${plateText}`
    );
  ...
  function handleAutoEndSession(reason) {
    ...
    if (!session) return;
    const zone =
      S.get.zones.find((z) => z.uid === session.zoneUid || z.id === session.zoneUid) ||
  ...
  function handleEndParking() {
    ...
    const zone =
      ...
      S.get.zones.find((z) => z.id === session.zone);
    const hourlyRate =
      zone && zone.price != null ? parseFloat(zone.price) : S.get.selectedZoneRate || 2.0;
    ...
    addNotification(
      'sessionEndedByUser',
      S.get.language === 'nl' ? 'Parkeersessie beëindigd' : 'Parking session ended',
  ...
  function saveNewPlate() {
    ...
    if (!rawVal)
      return toast(
        S.get.language === 'nl' ? 'Voer een kenteken in' : 'Please enter a license plate',
        'error'
      );
    ...
    if (Kenteken) {
      ...
      if (normalized.length > 8)
        return toast(
          S.get.language === 'nl' ? 'Kenteken te lang' : 'License plate too long',
          'error'
        );
      if (!/^[A-Z0-9]+$/.test(normalized))
        return toast(
          S.get.language === 'nl' ? 'Alleen letters en cijfers' : 'Letters and digits only',
          'error'
        );
    }
    ...
    if (!formatValid)
      return toast(
        formatError ||
          (S.get.language === 'nl' ? 'Ongeldig kentekenformaat' : 'Invalid license plate format'),
        'error'
      );
    if (S.get.plates.some((p) => p.text === normalized || p.id === normalized))
      return toast(
        S.get.language === 'nl' ? 'Kenteken bestaat al' : 'License plate already exists',
        'error'
      );

    const isFirst = S.get.plates.length === 0;
    const displayText =
      Kenteken && Kenteken.formatDisplay ? Kenteken.formatDisplay(normalized) : normalized;
    const newPlates = [
      ...S.get.plates,
      {
        id: normalized,
      ...
      Kenteken.lookupRDW(normalized)
        .then(function (result) {
          if (result.found && result.data) {
            const brand =
              (result.data.merk || '') +
              (result.data.handelsbenaming ? ' ' + result.data.handelsbenaming : '');
            toast(
              S.get.language === 'nl'
                ? 'Kenteken toegevoegd (gecontroleerd: ' + (brand.trim() || 'RDW') + ')'
                : 'License plate added (verified: ' + (brand.trim() || 'RDW') + ')',
              'success'
            );
  ...
  function updatePlate(id, newText, newDescription) {
    ...
    if (
      S.get.plates.some(
        (p) => p.id != id && p.text != id && (p.text === normalized || p.id === normalized)
      )
    )
      return toast(
        S.get.language === 'nl' ? 'Kenteken bestaat al' : 'License plate already exists',
  ...
  function checkPlateRDW() {
    ...
    if (!rawVal) {
      resultEl.textContent =
        S.get.language === 'nl' ? 'Voer eerst een kenteken in.' : 'Enter a license plate first.';
    ...
    if (!Kenteken) {
      resultEl.textContent =
        S.get.language === 'nl' ? 'Validatie niet beschikbaar.' : 'Validation not available.';
      resultEl.classList.add('plate-rdw-error');
      return;
    }
    ...
    if (!v.valid) {
      resultEl.textContent =
        v.errorMessage || (S.get.language === 'nl' ? 'Ongeldig kentekenformaat' : 'Invalid format');
      resultEl.classList.add('plate-rdw-error');
      return;
    }
    ...
    Kenteken.lookupRDW(v.normalized)
      .then(function (result) {
        ...
        if (result.found && result.data) {
          ...
          resultEl.textContent =
            (S.get.language === 'nl' ? 'Gevonden: ' : 'Found: ') + vehicleLabel;
  ...
  function setDefaultPlate() {
    ...
    if (Q8.UI && Q8.UI.showToast)
      Q8.UI.showToast(
        S.get.language === 'nl' ? 'Standaard kenteken bijgewerkt' : 'Default plate updated',
        'success'
      );
    else if (typeof window.showToast === 'function')
      window.showToast(
