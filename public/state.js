...
Q8.State = (function () {
  ...
  function update(changes) {
    ...
    if (typeof Q8.Utils.debug === "function") {
      ...
      if (changes.zones) {
        Q8.Utils.debug(
          "STATE",
          `Updated: ${keys} (Zones count: ${changes.zones.length})`,
        );
      } else {
  ...
  function load() {
    ...
    if (savedSession) {
      try {
        ...
        if (parsed) {
          ...
          if (isNaN(parsed.start.getTime()))
            console.warn("[PERSIST] Session start date invalid", parsed.start);
          if (parsed.end && isNaN(parsed.end.getTime()))
            console.warn("[PERSIST] Session end date invalid", parsed.end);
        }
    ...
    if (savedAuthPrefs) {
      try {
        ...
        _state.rememberMeUntil =
          typeof p.rememberMeUntil === "number" ? p.rememberMeUntil : null;
      } catch (e) {
    ...
    try {
      ...
      if (dd !== null) {
        ...
        if (!isNaN(n) && n >= 0)
          _state.defaultDurationMinutes = Math.min(1440, n);
      }
    } catch (e) {
      /* ignore */
    }
    ...
    try {
      ...
      if (d === "light" || d === "dark" || d === "system") _state.darkMode = d;
        ...
        if (legacy !== null)
          _state.darkMode = legacy === "true" ? "dark" : "light";
      }
    } catch (e) {
      /* ignore */
    }
  ...
  function savePlates() {
    try {
      ...
      console.warn("[PERSIST] Plates save failed", e);
    }
  }
  ...
  function saveAuthPrefs() {
    try {
      ...
      console.warn("[PERSIST] Auth prefs save failed", e);
    }
  }
  ...
  function clearRememberMe() {
    ...
    _state.rememberMeUntil = null;
    try {
      localStorage.removeItem("q8_auth_prefs_v1");
    } catch (e) {
      /* ignore */
    }
  }
  ...
  function saveNotifications() {
    try {
      localStorage.setItem(
        "q8_notifications_v1",
        JSON.stringify(_state.notifications),
      );
      localStorage.setItem(
        "q8_notif_settings_v1",
        JSON.stringify(_state.notificationSettings),
      );
    } catch (e) {
      console.warn("[PERSIST] Notifications save failed", e);
    }
  }
  ...
  function loadNotifications() {
    try {
      ...
      if (settings)
        _state.notificationSettings = {
          ..._state.notificationSettings,
          ...JSON.parse(settings),
        };
    } catch (e) {
      console.warn("[PERSIST] Notifications load failed", e);
    }
  }
  ...
  function saveFavorites() {
    try {
      ...
      console.warn("[PERSIST] Favorites save failed", e);
    }
  }
  ...
  function loadFavorites() {
    try {
      ...
      if (saved) {
        ...
        _state.favorites = (Array.isArray(parsed) ? parsed : []).map(
          (f, i) => ({
            zoneUid: f.zoneUid,
            zoneId: f.zoneId || f.zoneUid,
            name:
              typeof f.name === "string"
                ? f.name.trim() || undefined
                : undefined,
      ...
      console.warn("[PERSIST] Favorites load failed", e);
    }
  }
  ...
  function setDarkMode(value) {
    ...
    applyThemeFromPref();
    try {
      localStorage.setItem("q8_dark_v2", _state.darkMode);
    } catch (e) {}
  }
  ...
  function saveDefaultDuration() {
    try {
      localStorage.setItem(
        "q8_default_duration",
        String(_state.defaultDurationMinutes),
      );
    } catch (e) {}
  }
