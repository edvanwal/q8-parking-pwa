...
(function () {
  ...
  function showSection(sectionId) {
    ...
    $all('.nav-item').forEach((n) =>
      n.classList.toggle('active', n.dataset.section === sectionId.replace('section-', ''))
    );
    const titles = {
      dashboard: 'Dashboard',
      users: 'Users',
  ...
  function saveTenantSettings(settings) {
    return db
      ...
      .then(() => writeAuditLog('tenant_settings_saved', settings));
  }
  ...
  function renderAuditLog(items) {
    ...
    tbody.innerHTML = items
      .map((item) => {
        ...
        const dateStr = ts
          ? ts.toLocaleString('nl-NL', { dateStyle: 'short', timeStyle: 'short' })
          : '-';
    ...
    tbody.innerHTML = sessions
      .map((s) => {
        ...
        const startStr = start
          ? start.toLocaleString('nl-NL', { dateStyle: 'short', timeStyle: 'short' })
          : '-';
        return `
        <tr>
  ...
  function populateDriverSelect(users) {
    ...
    select.innerHTML =
      ...
      drivers
        .map((u) => `<option value="${u.id}">${u.displayName || u.email} (${u.email})</option>`)
        .join('');
  }
  ...
  function initEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (removePlate) {
        ...
        removePlateFromUser(uid, plateId)
          .then(() => {
    ...
    $('btn-save-plates-settings').addEventListener('click', () => {
      ...
      updateDriverSettings(uid, settings)
        .then(() => {
          toast('Kentekeninstellingen opgeslagen');
          loadUsers((u) => {
            renderUsers(u);
            renderPlatesPanel(u.find((x) => x.id === uid));
          });
        })
        .catch((err) => toast(err.message));
    });
    ...
    $('btn-bulk-add-plates').addEventListener('click', () => {
      ...
      const plates = text
        ...
        .filter((p) => p.length >= 6);
      if (plates.length === 0)
        return toast('Voer minimaal één kenteken in (één per regel of komma-gescheiden)');
      let done = 0,
        failed = 0;
      const addedPlates = [];
      const next = () => {
        if (plates.length === 0) {
          if (addedPlates.length > 0)
            writeAuditLog('plates_bulk_added', {
              userId: uid,
              plates: addedPlates,
              count: addedPlates.length,
            });
        ...
        addPlateToUser(uid, plate)
          .then(() => {
            done++;
            addedPlates.push(plate);
            next();
          })
          .catch(() => {
            failed++;
