...
Q8.UI = (function () {
  ...
  function renderParkingView() {
    ...
    if (favStrip && !hideSearchUI) {
      ...
      if (lastUsedUid)
        lastUsedZone = state.zones.find(
          (z) => z.uid === lastUsedUid || z.id === lastUsedUid || z.id === lastUsedZoneId
        );
      ...
      const favZones = favs
        .map((f) => {
          const zone = state.zones.find(
            (z) => z.uid === f.zoneUid || z.id === f.zoneUid || z.id === f.zoneId
          );
    ...
    if (zonesLoadingEl) {
      ...
      if (textEl)
        textEl.textContent =
          state.language === 'nl' ? 'Parkeerzones laden...' : 'Loading parking zones...';
      const subEl = zonesLoadingEl.querySelector('.zones-loading-subtitle');
      if (subEl)
        subEl.textContent = state.language === 'nl' ? 'Zones worden geladen…' : 'Loading zones…';
    }
    if (zonesErrorEl) {
    ...
    if (isActive && state.session) {
      ...
      const plateDisplay =
        state.session.plate && state.session.plate.trim() ? state.session.plate.trim() : null;
      const fallbackPlate =
    ...
    document.querySelectorAll('.dark-mode-opt').forEach((btn) => {
      btn.setAttribute(
        'aria-pressed',
        btn.getAttribute('data-pref') === (state.darkMode || 'system') ? 'true' : 'false'
      );
    });
  }
  ...
  function renderZoneSheet() {
    ...
    if (elZoneId) {
      ...
      if (zone) {
        ...
        if (sheetZoneHeader) {
          const firstChild =
            sheetZoneHeader.querySelector('.sheet-zone-title-row') || sheetZoneHeader.children[0];
    ...
    const zoneMax =
      zone && zone.max_duration_mins && zone.max_duration_mins > 0 ? zone.max_duration_mins : 1440;
  ...
  function renderRatesList(container, rates) {
    ...
    const finalRates =
      unique.length > 0 ? unique : [{ time: `Geen tarieven voor ${todayNL}`, price: '' }];
  ...
  function renderLoginUI() {
    ...
    document.querySelectorAll('.lang-btn').forEach((btn) => {
      ...
      btn.className =
        state.language === lang ? 'btn btn-primary lang-btn' : 'btn btn-outline lang-btn';
    });
  ...
  function renderCarSpecs() {
    ...
    plates.forEach(function (plate) {
      ...
      if (!Kenteken || !Kenteken.getVehicleSpecs) {
        if (loadingEl)
          loadingEl.textContent = nl ? 'Specs niet beschikbaar.' : 'Specs not available.';
        return;
      }
  ...
  function renderNotifications() {
    ...
    const settings = [
      { key: 'sessionStarted', label: nl ? 'Parkeersessie gestart' : 'Parking session started' },
      {
        ...
        label: nl ? 'Parkeersessie verloopt binnenkort' : 'Parking session expiring soon',
        hasInterval: true,
      },
      {
        key: 'sessionEndedByUser',
        label: nl ? 'Sessie beëindigd (eindtijd)' : 'Session ended (end time reached)',
      },
      {
        key: 'sessionEndedByMaxTime',
        label: nl ? 'Sessie beëindigd (max parkeertijd)' : 'Session ended (max parking time)',
      },
    ];
    ...
    const fmtDate = (iso) => {
      ...
      return sameDay
        ? d.toLocaleTimeString(nl ? 'nl-NL' : 'en-GB', { hour: '2-digit', minute: '2-digit' })
    ...
    if (summaryEl) {
      if (hasDateFilter || hasVehicleFilter || hasCustomRange) {
        ...
        if (hasDateFilter && filters.dateRange !== 'custom') {
          ...
          parts.push(
            (nl ? 'Datum: ' : 'Date: ') + (labels[filters.dateRange] || filters.dateRange)
          );
        }
        if (hasCustomRange)
          parts.push(
        ...
        if (hasVehicleFilter)
          parts.push((nl ? 'Voertuig: ' : 'Vehicle: ') + filters.vehicles.join(', '));
    ...
    if (isAddressMode) {
      ...
      if (state.geocodeLoading) {
        container.innerHTML =
          ...
          (state.language === 'nl' ? 'Zoeken op adres...' : 'Searching by address...') +
          '</div>';
        return;
      }
  ...
  function updateActiveTimerDisplay() {
    ...
    const zone =
      state.zones.find((z) => z.uid === state.session.zoneUid || z.id === state.session.zoneUid) ||
    ...
    const expiringMins =
      (state.notificationSettings && state.notificationSettings.expiringSoonMinutes) || 10;
  ...
  function createPriceMarkersOverlay() {
    ...
    PriceMarkersOverlay.prototype.draw = function () {
      ...
      (state.zones || []).forEach((z) => {
        ...
        const priceLabel =
          z.display_label ||
          (typeof z.price === 'number' ? z.price.toFixed(2).replace('.', ',') : String(z.price));
        ...
        const addr =
          (z.street
            ? z.street + (z.houseNumber ? ' ' + z.houseNumber : '') + (z.city ? ', ' + z.city : '')
