...
Q8.UI = (function () {
  ...
  function update() {
    ...
    if (state.screen === "plates") {
      ...
      const btnAdd = document.querySelector(
        '[data-target="modal-add-plate"] span',
      );
      if (btnAdd) {
        btnAdd.innerText =
          state.language === "nl"
            ? "Nieuw kenteken toevoegen"
  ...
  function updateOnboardingOverlay() {
    ...
    let done = false;
    try {
      done = !!localStorage.getItem("q8_onboarding_done");
    } catch (e) {
      /* ignore */
    }
    if (done) {
  ...
  function renderParkingView() {
    ...
    if (activeParking)
      activeParking.style.display = isActive ? "block" : "none";
    ...
    if (noPlatesHint) {
      const showHint =
        !isActive && !state.selectedZone && state.plates.length === 0;
      ...
      if (showHint) {
        ...
        if (textEl)
          textEl.textContent = nl
            ? "Voeg een kenteken toe om te kunnen parkeren."
            : "Add a license plate to start parking.";
        if (ctaEl)
          ctaEl.textContent = nl ? "Kenteken toevoegen" : "Add license plate";
      }
    }
    ...
    if (favStrip && !hideSearchUI) {
      ...
      try {
        ...
        lastUsedZoneId =
          localStorage.getItem("q8_last_used_zone_id") || lastUsedUid;
      } catch (e) {
        /* ignore */
      }
      if (lastUsedUid)
        lastUsedZone = state.zones.find(
          (z) =>
            z.uid === lastUsedUid ||
            z.id === lastUsedUid ||
            z.id === lastUsedZoneId,
      ...
      const favZones = favs
        .map((f) => {
          const zone = state.zones.find(
            (z) =>
              z.uid === f.zoneUid || z.id === f.zoneUid || z.id === f.zoneId,
          );
          return zone
            ? { ...zone, zoneUid: zone.uid || zone.id, favName: f.name }
            : null;
        })
        .filter(Boolean);
      ...
      if (hasLast || hasFavs) {
        ...
        favZones.forEach((z) => {
          ...
          const title =
            z.favName && z.favName.trim() ? z.favName.trim() : zoneId;
    ...
    if (zonesLoadingEl) {
      ...
      if (subEl)
        subEl.textContent =
          state.language === "nl" ? "Zones worden geladen…" : "Loading zones…";
    }
    if (zonesErrorEl) {
      ...
      const nl = state.language === "nl";
      if (msgEl)
        msgEl.textContent = nl
          ? "Kan parkeerzones niet laden. Controleer uw internetverbinding."
          : "Could not load parking zones. Check your internet connection.";
      const retryBtn = zonesErrorEl.querySelector(
        '[data-action="retry-load-zones"]',
      );
    ...
    if (isActive && state.session) {
      ...
      const plateDisplay =
        state.session.plate && state.session.plate.trim()
          ? state.session.plate.trim()
          : null;
      const fallbackPlate =
      ...
      if (elLabel)
        elLabel.innerText =
          plateDisplay || (fallbackPlate ? fallbackPlate.text : "");
      ...
      if (elEnd) {
        elEnd.innerText = state.session.end
          ? fmt(state.session.end)
          : "Until stopped";
      }
      const btnMinus = document.querySelector(
        '.active-end-btn[data-delta="-30"]',
      );
    ...
    document.querySelectorAll(".search-mode-btn").forEach((btn) => {
      btn.classList.toggle(
        "active",
        btn.getAttribute("data-mode") === state.searchMode,
      );
    });
    ...
    document.querySelectorAll(".side-menu .menu-lang-btn").forEach((btn) => {
      btn.classList.toggle(
        "active",
        btn.getAttribute("data-lang") === state.language,
      );
    });
    document.querySelectorAll(".dark-mode-opt").forEach((btn) => {
      btn.setAttribute(
        "aria-pressed",
        btn.getAttribute("data-pref") === (state.darkMode || "system")
          ? "true"
          : "false",
      );
    });
  ...
  function renderZoneSheet() {
    ...
    // Find zone object
    const zone =
      state.zones.find((z) => z.uid === state.selectedZone) ||
    ...
    if (elZoneId) {
      ...
      if (zone) {
        ...
        const detailsHTML = `
                ...
                `;
        const sheetZoneHeader = document.querySelector(
          "#sheet-zone .sheet-zone-header",
        );
        if (sheetZoneHeader) {
          const firstChild =
            sheetZoneHeader.querySelector(".sheet-zone-title-row") ||
            sheetZoneHeader.children[0];
          if (firstChild)
            firstChild.insertAdjacentHTML("afterend", detailsHTML);
        }
      }
    }
    ...
    // Buttons
    const btnSheetPlus =
      document.querySelector('.sheet-duration-control [data-delta="15"]') ||
      document.querySelector('.sheet-duration-control [data-delta="30"]') ||
      document.querySelector('.sheet-duration-control [data-delta="60"]');
    const btnSheetMinus =
      document.querySelector('.sheet-duration-control [data-delta="-15"]') ||
      document.querySelector('.sheet-duration-control [data-delta="-30"]') ||
      document.querySelector('.sheet-duration-control [data-delta="-60"]');
    ...
    if (elDur) {
      if (state.duration === 0) {
        ...
        elDur.innerText =
          m === 0 ? String(h) : `${h}h ${m.toString().padStart(2, "0")}m`;
      }
    }

    const zoneMax =
      zone && zone.max_duration_mins && zone.max_duration_mins > 0
    ...
    if (state.duration >= zoneMax) {
      ...
      if (!elMaxMsg) {
        ...
        msg.style.cssText =
          "color: #D92D20; font-size: 0.75rem; font-weight: 600; margin-top: 4px; text-align: center; width: 100%;";
    ...
    if (durationLabelEl) {
      durationLabelEl.textContent =
        state.language === "nl" ? "Duur" : "Parking duration";
    }
    ...
    if (defaultDurLabel) {
      defaultDurLabel.textContent =
        state.language === "nl" ? "Standaard duur" : "Default duration";
    }
    ...
    const isFav =
      ...
      (state.favorites || []).some(
        (f) => f.zoneUid === zoneUid || (f.zoneId === zoneId && !f.zoneUid),
      );
  ...
  function renderRatesList(container, rates) {
    ...
    const filtered = rates.filter((r) => {
      ...
      return (
        t.includes(todayNL.toLowerCase()) ||
        t.includes("dagelijks") ||
        t.includes("daily") ||
        t === "24/7" ||
        t.includes("check zone")
      );
    });
    ...
    const finalRates =
      unique.length > 0
        ? unique
        : [{ time: `Geen tarieven voor ${todayNL}`, price: "" }];

    container.innerHTML = finalRates
      .map((r) => {
        let displayTime = r.time
          .replace(new RegExp(todayNL, "gi"), "")
          .replace(/Dagelijks/gi, "")
          .trim();
        const timeMatch = displayTime.match(
          /(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})/,
        );
        if (timeMatch) {
          const fmt = (h, m) =>
            (parseInt(h, 10) < 10 ? String(h).replace(/^0/, "") : h) + ":" + m;
          displayTime = `${fmt(timeMatch[1], timeMatch[2])} – ${fmt(timeMatch[3], timeMatch[4])}`;
        }
        const priceStr =
          r.price != null && r.price !== "" ? String(r.price) : "";
        const isFree =
          !priceStr ||
        ...
        if (r.detail) {
          const lines = r.detail
            .split("|")
            .filter((l) => l && l.trim() !== "" && l !== "Vrij parkeren");
          if (lines.length > 0) {
  ...
  function renderPlates() {
    ...
    const sorted = [...state.plates].sort((a, b) =>
    ...
    sorted.forEach((p) => {
      ...
      div.innerHTML = `
            ...
            `;
      div.querySelectorAll(".ptr-enabled").forEach((el) => {
  ...
  function renderCarSpecs() {
    ...
    const Kenteken =
      typeof Q8 !== "undefined" && Q8.Kenteken ? Q8.Kenteken : null;
    const nl = state.language === "nl";
    if (!list) return;
    if (intro)
      intro.innerText = nl
        ? "Specs van je ingevulde kentekens (RDW)."
        : "Specs for your registered license plates (RDW).";
    ...
    if (plates.length === 0) {
      list.innerHTML =
        ...
        (nl
          ? "Voeg eerst kentekens toe onder License plates."
          : "Add license plates first under License plates.") +
        "</p>";
      return;
    }
    ...
    plates.forEach(function (plate) {
      const normalized =
        Kenteken && Kenteken.normalize
          ? Kenteken.normalize(plate.text || plate.id)
          : plate.id || (plate.text || "").replace(/[\s\-]/g, "").toUpperCase();
      const card = document.createElement("div");
      card.className = "card mb-lg car-specs-card";
      card.style.padding = "16px 20px";
      card.innerHTML =
        '<div class="font-bold text-lg mb-md" style="display:flex;align-items:center;gap:8px;">' +
        (plate.text || plate.id) +
        '<span class="car-specs-loading text-secondary text-sm" style="font-weight:400;">' +
        (nl ? "Laden..." : "Loading...") +
        "</span></div>" +
      ...
      if (!Kenteken || !Kenteken.getVehicleSpecs) {
        if (loadingEl)
          loadingEl.textContent = nl
            ? "Specs niet beschikbaar."
            : "Specs not available.";
        return;
      }
      ...
      Kenteken.getVehicleSpecs(normalized).then(function (result) {
        ...
        if (!result.found || result.error) {
          ...
          fieldsEl.innerHTML =
            ...
            (result.error
              ? nl
                ? "RDW tijdelijk niet bereikbaar."
                : "RDW temporarily unavailable."
              : nl
                ? "Niet gevonden in RDW."
                : "Not found in RDW.") +
            "</p>";
          return;
        }
        ...
        const kleur =
          [s.eerste_kleur, s.tweede_kleur]
            .filter(Boolean)
            .join(", ")
            .replace(/Niet geregistreerd/gi, "") || "—";
        const gewicht =
          s.massa_ledig_voertuig || s.toegestane_maximum_massa_voertuig || "—";
        const rows = [
          [labels.merk, s.merk || "—"],
          ...
          [
            labels.elektrisch,
            s.elektrisch ? (nl ? "Ja" : "Yes") : nl ? "Nee" : "No",
          ],
        ...
        fieldsEl.innerHTML = rows
          .map(function (r) {
            return (
              '<div class="flex justify-between gap-md" style="align-items:baseline;"><span class="text-secondary text-sm">' +
              r[0] +
              '</span><span class="text-main font-medium">' +
              (r[1] || "—") +
              "</span></div>"
            );
          })
  ...
  function renderFavorites() {
    ...
    if (intro)
      intro.innerText =
        state.language === "nl"
          ? "Je favoriete parkeerzones. Sleep om volgorde te wijzigen."
          : "Your favorite parking zones. Drag to reorder.";
    const rateDisclaimer =
      state.language === "nl"
        ? "Tarieven zijn indicatief (RDW Open Data)"
        : "Rates are indicative (RDW Open Data)";
    ...
    const gripSvg =
      '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="opacity:0.5;"><path d="M8 6h2v2H8V6zm0 5h2v2H8v-2zm0 5h2v2H8v-2zm5-10h2v2h-2V6zm0 5h2v2h-2v-2zm0 5h2v2h-2v-2z"/></svg>';
    const editSvg =
    ...
    list.innerHTML =
      favorites
        ...
        .join("") +
      `<div class="rate-disclaimer text-secondary text-xs" style="padding:12px 20px 0; line-height:1.3;">${rateDisclaimer}</div>`;

    list.querySelectorAll(".ptr-enabled").forEach((el) => {
      el.style.pointerEvents = "auto";
    });
    ...
    list.querySelectorAll(".fav-card-draggable").forEach((el, idx) => {
      ...
      el.addEventListener("dragover", (e) => {
        ...
        if (_draggedIdx !== null && _draggedIdx !== idx)
  ...
  function renderNotifications() {
    ...
    const settings = [
      {
        key: "sessionStarted",
        label: nl ? "Parkeersessie gestart" : "Parking session started",
      },
      {
        key: "sessionExpiringSoon",
        label: nl
          ? "Parkeersessie verloopt binnenkort"
          : "Parking session expiring soon",
        hasInterval: true,
      },
      {
        key: "sessionEndedByUser",
        label: nl
          ? "Sessie beëindigd (eindtijd)"
          : "Session ended (end time reached)",
      },
      {
        key: "sessionEndedByMaxTime",
        label: nl
          ? "Sessie beëindigd (max parkeertijd)"
          : "Session ended (max parking time)",
      },
    ];
    ...
    const typeLabel = (t) => {
      ...
      if (t === "sessionEndedByUser")
        return nl ? "Beëindigd (tijd)" : "Ended (time)";
      if (t === "sessionEndedByMaxTime")
        return nl ? "Beëindigd (max)" : "Ended (max)";
      return t;
    };
    const fmtDate = (iso) => {
      ...
      return sameDay
        ? d.toLocaleTimeString(nl ? "nl-NL" : "en-GB", {
            hour: "2-digit",
            minute: "2-digit",
          })
        : d.toLocaleDateString(nl ? "nl-NL" : "en-GB", {
            day: "numeric",
            month: "short",
            hour: "2-digit",
            minute: "2-digit",
  ...
  function renderQuickPlateSelector() {
    ...
    const activePlateId =
      ...
      (state.plates.find((p) => p.default) || state.plates[0])?.id;
  ...
  function renderHistory() {
    ...
    if (summaryEl) {
      if (hasDateFilter || hasVehicleFilter || hasCustomRange) {
        ...
        if (hasDateFilter && filters.dateRange !== "custom") {
          const labels = {
            week: nl ? "Laatste 7 dagen" : "Last 7 days",
            "30days": nl ? "Laatste 30 dagen" : "Last 30 days",
            month: nl ? "Deze maand" : "This month",
          };
          parts.push(
            (nl ? "Datum: " : "Date: ") +
              (labels[filters.dateRange] || filters.dateRange),
          );
        }
        if (hasCustomRange)
          parts.push(
        ...
        if (hasVehicleFilter)
          parts.push(
            (nl ? "Voertuig: " : "Vehicle: ") + filters.vehicles.join(", "),
          );
    ...
    if (filters.vehicles.length > 0) {
      filteredHistory = filteredHistory.filter((h) =>
        filters.vehicles.includes(h.plate),
      );
    }
    ...
    if (filteredHistory.length === 0) {
      ...
      const subtitle = nl
        ? "Hier verschijnen je sessies na het beëindigen."
        : "Your sessions will appear here after you end them.";
      list.innerHTML =
        '<div class="history-empty-state"><p class="history-empty-state-title">' +
        title +
        '</p><p class="history-empty-state-subtitle">' +
        subtitle +
        "</p></div>";
    } else {
      filteredHistory.forEach((h) => {
        const zone = state.zones.find(
          (z) => z.id === h.zone || z.uid === h.zoneUid || z.uid === h.zone,
        );
        const zoneUid = zone ? zone.uid || zone.id : h.zoneUid || h.zone;
        const zoneId = zone ? zone.id : h.zone;
        const isFav = (state.favorites || []).some(
          (f) => f.zoneUid === zoneUid || f.zoneId === zoneId,
        );
        const div = document.createElement("div");
        ...
        div.setAttribute(
          "data-price",
          zone && zone.price != null ? zone.price : "",
        );
        div.setAttribute(
          "data-rates",
          JSON.stringify(zone && zone.rates ? zone.rates : []),
        );
        div.innerHTML = `
    ...
    if (
      ...
      const activeCount =
        ...
        (filters.customStart ? 1 : 0) +
        (filters.customEnd ? 1 : 0);
      const badge =
        activeCount > 0
          ? `<span class="filter-badge-count">${activeCount}</span>`
          : "";
  ...
  function renderHistoryFilters() {
    ...
    if (
      state.activeOverlay !== "sheet-filter" &&
      state.activeOverlay !== "sheet-filters"
    )
      return;
  ...
  function renderSearchResults() {
    ...
    const isAddressMode =
      Q8.Services &&
      Q8.Services.detectSearchMode &&
      Q8.Services.detectSearchMode(query) === "address";
    const minLen = isAddressMode ? 3 : 2;
    const zoneSelectedOrActive = !!state.selectedZone || !!state.session;
    const shouldShow =
      state.screen === "parking" &&
      !zoneSelectedOrActive &&
      state.searchQuery.length >= minLen &&
    ...
    if (favQuick) {
      ...
      favQuick.className = showFavQuick
        ? "search-results-panel search-results-panel--pill"
        : "hidden";
      if (showFavQuick) {
        ...
        const favZones = favs
          ...
          .filter(Boolean);
        const rateDisclaimer =
          '<div class="rate-disclaimer text-secondary text-xs" style="padding:8px 16px 12px; line-height:1.3;">' +
          (state.language === "nl"
            ? "Tarieven zijn indicatief (RDW Open Data)"
            : "Rates are indicative (RDW Open Data)") +
          "</div>";
        favQuick.innerHTML = `
                  ...
                  ${
                    favZones.length === 0
                      ? '<div class="text-secondary text-sm" style="padding:12px 16px;">' +
                        (state.language === "nl"
                          ? "Geen favorieten in deze sessie"
                          : "No favorites loaded") +
                        "</div>"
                      : favZones
                          .map((z) => {
    ...
    if (isAddressMode) {
      ...
      if (state.geocodeLoading) {
        container.innerHTML =
          '<div class="text-secondary text-sm" style="padding:16px;">' +
          (state.language === "nl"
            ? "Zoeken op adres..."
      ...
      if (geoMatches.length === 0) {
        container.innerHTML =
          '<div class="text-secondary text-sm" style="padding:16px;">' +
          (state.language === "nl"
            ? "Geen parkeerzones in de buurt gevonden."
            : "No parking zones found nearby.") +
          "</div>";
        return;
      }
      const favUids = new Set(
        (state.favorites || []).map((f) => f.zoneUid || f.zoneId),
      );
      container.innerHTML =
        geoMatches
          .map((z) => {
    ...
    if (matches.length === 0) {
      ...
      const noResultsMsg =
        state.language === "nl"
          ? query
            ? `Geen zones gevonden voor "${query}". Probeer een ander zone-nummer of adres.`
            : "Geen zones gevonden. Probeer een zone-nummer of adres."
          : query
            ? `No zones found for "${query}". Try a different zone number or address.`
            : "No zones found. Try a zone number or address.";
      container.innerHTML =
        '<div class="search-result-empty" style="padding:20px 20px 24px; text-align:center; color:var(--text-secondary); font-size:0.95rem; line-height:1.5;">' +
        noResultsMsg +
        "</div>";
      return;
    }
    ...
    container.innerHTML = matches
      .map((z) => {
        ...
        const displayAddr = regex
          ? line.replace(regex, '<strong class="search-highlight">$1</strong>')
          : line;
        const isFav = favUids.has(z.uid) || favUids.has(z.id);
        const uid = z.uid || z.id;
        const zoneIdAttr = z.id || z.uid || "";
    ...
    container.innerHTML +=
      ...
      (state.language === "nl"
        ...
        : "Rates are indicative (RDW Open Data)") +
      "</div>";
    container.querySelectorAll(".ptr-enabled").forEach((el) => {
      el.style.pointerEvents = "auto";
    });
  }
  ...
  function renderInstallGate() {
    ...
    const t = {
      ...
      subtitle: isEn
        ? "Add Q8 Parking to your home screen for the best experience"
        : "Voeg Q8 Parking toe aan je startscherm voor de beste ervaring",
      whyTitle: isEn ? "Why install?" : "Waarom installeren?",
      why1: isEn
        ? "Open faster from your home screen"
        : "Sneller openen vanaf je startscherm",
      why2: isEn
        ? "Notifications when parking is about to end"
        : "Meldingen wanneer parkeren bijna afloopt",
      ...
      iosStep2: isEn
        ...
        : "Tik op het <b>Deel</b>-icoon",
      iosStep2Sub: isEn
        ? "(square with arrow pointing up, bottom of screen)"
        : "(vierkant met pijl omhoog, onderaan)",
      iosStep3: isEn
        ? "Tap <b>Add to Home Screen</b>"
        : "Tik op <b>Zet op beginscherm</b>",
      iosStep4: isEn
        ? "Tap <b>Add</b> (top right)"
        : "Tik op <b>Voeg toe</b> (rechtsboven)",
      androidTitle: isEn ? "Install on Android" : "Installeren op Android",
      androidStep1: isEn
        ? "Open this page in <b>Chrome</b>"
        : "Open deze pagina in <b>Chrome</b>",
      androidStep2: isEn
        ? "Tap the <b>menu</b> (⋮, top right)"
        : "Tik op het <b>menu</b> (⋮, rechtsboven)",
      androidStep3: isEn
        ? "Tap <b>Install app</b> or <b>Add to Home screen</b>"
        : "Tik op <b>App installeren</b> of <b>Toevoegen aan startscherm</b>",
      androidStep4: isEn
        ? "Confirm with <b>Install</b> or <b>Add</b>"
        : "Bevestig met <b>Installeren</b> of <b>Toevoegen</b>",
      bothNote: isEn
        ? "Use Safari on iOS, Chrome on Android."
        : "Gebruik Safari op iOS, Chrome op Android.",
    };
  ...
  function updateActiveTimerDisplay() {
    ...
    const zone =
      state.zones.find(
        (z) =>
          z.uid === state.session.zoneUid || z.id === state.session.zoneUid,
      ) || state.zones.find((z) => z.id === state.session.zone);
    const maxDurMins =
      zone && zone.max_duration_mins && zone.max_duration_mins > 0
        ? zone.max_duration_mins
        : 1440;
    ...
    if (!endDate) {
      if (elLabel)
        elLabel.innerText =
          state.language === "nl" ? "Tijd geparkeerd" : "Time";
      const elapsed = now - startDate;
      if (elapsed >= maxDurMins * 60000) {
        if (Q8.Services && Q8.Services.handleAutoEndSession)
          Q8.Services.handleAutoEndSession("sessionEndedByMaxTime");
        return;
      }
    ...
    if (elLabel)
      elLabel.innerText =
        state.language === "nl" ? "Resterende tijd" : "Time left";
    const diff = endDate.getTime() - now.getTime();

    if (diff <= 0) {
      if (Q8.Services && Q8.Services.handleAutoEndSession)
        Q8.Services.handleAutoEndSession("sessionEndedByUser");
      return;
    }
    ...
    const expiringMins =
      (state.notificationSettings &&
        state.notificationSettings.expiringSoonMinutes) ||
      10;
    if (
      diff <= expiringMins * 60 * 1000 &&
      ...
      if (Q8.Services && Q8.Services.addNotification) {
        const msg =
          state.language === "nl"
            ? `Parkeersessie verloopt over ${expiringMins} minuten`
            : `Parking session expires in ${expiringMins} minutes`;
        Q8.Services.addNotification(
          "sessionExpiringSoon",
          msg,
          `${state.session.zone || "?"} · ${state.session.plate || "?"}`,
        );
      }
    }
      ...
      (state.zones || []).forEach((z) => {
        ...
        const priceLabel =
          ...
          (typeof z.price === "number"
            ...
            : String(z.price));
        const priceText =
          z.price === 0 ? "Free" : "€ " + priceLabel.substring(0, 6);
        const isSelected =
          (z.uid && z.uid === state.selectedZone) ||
          (z.id && String(z.id) === String(state.selectedZone));
        ...
        el.className =
          "q8-price-marker" + (isSelected ? " q8-price-marker--selected" : "");
        el.textContent = priceText;
        const addr =
          (z.street
            ? z.street +
              (z.houseNumber ? " " + z.houseNumber : "") +
              (z.city ? ", " + z.city : "")
            : null) || "Zone " + (z.id || z.uid);
  ...
  function populateConfirmStartModal(zoneId, plateText, options) {
    ...
    const cancelBtn = document.querySelector(
      '#modal-confirm-start [data-action="close-overlay"]',
    );
    const okBtn = document.querySelector(
      '#modal-confirm-start [data-action="confirm-start-session"]',
    );
    if (titleEl)
      titleEl.textContent = nl
        ? "Bevestig parkeersessie"
        : "Confirm parking session";
    const descEl = document.getElementById("confirm-start-desc");
    if (descEl)
      descEl.textContent = nl
        ? "Controleer zonenummer en kenteken alvorens te starten."
        : "Please verify zone number and license plate before starting.";
    if (zoneEl) zoneEl.textContent = zoneId || "—";
    if (plateEl) plateEl.textContent = plateText || "—";
    const zoneLabel = document.querySelector(
      "#modal-confirm-start .flex.justify-between .text-secondary",
    );
    if (zoneLabel) zoneLabel.textContent = nl ? "Zone" : "Zone";
    const plateLabel = document.querySelector(
      '#modal-confirm-start [data-action="open-plate-selector-from-confirm"] .text-secondary',
    );
    if (plateLabel) plateLabel.textContent = nl ? "Kenteken" : "License plate";
    const plateRow = document.querySelector(
      '#modal-confirm-start [data-action="open-plate-selector-from-confirm"]',
    );
    if (plateRow)
      plateRow.setAttribute(
        "title",
        nl ? "Wijzig kenteken" : "Change license plate",
      );
