  ...
  if (uid) {
    ...
    if (nSettings.sessionEndedByMaxTime !== false) {
      ...
      await sendPushToUser(
        uid,
        'Q8 Parking',
        `Parkeersessie gestopt omdat de eindtijd bereikt is · ${zone} · ${plate}`,
        {
...
exports.onSessionCreated = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    if (!uid) return null;
    const settings =
      (await db.collection('users').doc(uid).get()).data()?.notificationSettings || {};
    ...
    try {
      ...
      for (const sessionDoc of sessionsSnapshot.docs) {
        ...
        if (endTime && now >= endTime) {
          ...
          console.log(
            `Session ${sessionDoc.id}: duration expired (end time: ${endTime.toISOString()})`
          );
        }
 ...
 */
exports.triggerAutoStop = functions.region('europe-west1').https.onCall(async (data, context) => {
  ...
  try {
    ...
    const sessionsSnapshot = await db.collection('sessions').where('status', '==', 'active').get();
