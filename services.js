...
Q8.Services = (function () {
      ...
      .then((snap) => {
        ...
        if (session.start && !isNaN(session.start.getTime())) {
          ...
          if (U && U.debug)
            U.debug(
              "AUTH",
              "Session restored from Firestore after localStorage cleared",
            );
        }
      })
      .catch(() => {});
  ...
  function findAndSelectZoneAtLocation(lat, lng) {
    ...
    const withDist = zones
      ...
      .map((z) => ({
        ...z,
        _dist: haversineKm(lat, lng, parseFloat(z.lat), parseFloat(z.lng)),
      }))
    ...
    if (nearest && nearest._dist < ZONE_PRESELECT_RADIUS_KM) {
      ...
      S.update({
        selectedZone: uid,
        selectedZoneRate:
          nearest.price != null ? parseFloat(nearest.price) : 2.0,
  ...
  function geocodeAndSearch(query) {
    ...
    const key =
      typeof firebaseConfig !== "undefined" && firebaseConfig.googleMapsApiKey
      ...
      .onSnapshot(
        (snapshot) => {
          ...
          snapshot.forEach((doc) => {
            ...
            const startDate =
              d.start &&
              (d.start.toDate ? d.start.toDate() : new Date(d.start));
            const endDate =
              d.end && (d.end.toDate ? d.end.toDate() : new Date(d.end));
            ...
            items.push({
              ...
              street: d.street || "",
              date: startDate
                ? `${dd(startDate.getDate())}-${dd(startDate.getMonth() + 1)}-${startDate.getFullYear()}`
                : "",
              start: startDate
                ? `${dd(startDate.getHours())}:${dd(startDate.getMinutes())}`
                : "",
              end: endDate
                ? `${dd(endDate.getHours())}:${dd(endDate.getMinutes())}`
                : "",
  ...
  const NON_STREET_PATTERNS =
    /garage|p\+r|p&r|carpool|transferium|parkeergarage/i;
  function isStreetParkingZone(z) {
    ...
    const rates = z.rates || [];
    if (rates.some((r) => NON_STREET_PATTERNS.test(r.detail || "")))
      return false;
    return true;
  }
      ...
      db.collection("zones")
        ...
        .onSnapshot(
          (snapshot) => {
            ...
            diag("onSnapshot", "received", {
              raw: raw.length,
              streetOnly: zones.length,
            });
            if (zones.length > 0) {
              requestAnimationFrame(() => {
                ...
                if (U && U.debug)
                  U.debug("DATA", `Live sync: ${zones.length} zones loaded.`);
                diag("onSnapshot", "zones-updated");

                if (Q8.UI && typeof Q8.UI.renderMapMarkers === "function")
                  Q8.UI.renderMapMarkers();
                else if (typeof window.renderMapMarkers === "function")
                  window.renderMapMarkers();

                if (Q8.UI && typeof Q8.UI.centerMapOnZones === "function")
                  Q8.UI.centerMapOnZones();
                else if (typeof window.centerMapOnZones === "function")
                  window.centerMapOnZones();

                if (S.get.userLocation)
                  findAndSelectZoneAtLocation(
                    S.get.userLocation.lat,
                    S.get.userLocation.lng,
                  );
  ...
  function setScreen(name) {
    ...
    if (name === "parking" && !S.get.installMode.active) {
      if (Q8.UI && typeof Q8.UI.initGoogleMap === "function")
        Q8.UI.initGoogleMap();
      ...
      requestAnimationFrame(() => {
        if (Q8.UI && typeof Q8.UI.ensureMapResized === "function")
          Q8.UI.ensureMapResized();
  ...
  function tryOpenOverlay(id, contextData = null) {
    const allowedSwitches = [
      "menu-overlay",
      "sheet-plate-selector",
      "sheet-zone",
      "modal-add-plate",
      "modal-edit-plate",
      "modal-forgot-password",
      "modal-confirm-delete-plate",
      "modal-confirm-daypass",
      "modal-confirm-start",
    ];
    ...
    if (
      S.get.activeOverlay &&
      S.get.activeOverlay !== id &&
      !allowedSwitches.includes(id)
    ) {
      return;
    }
    ...
    if (id === "sheet-zone") {
      ...
      if (contextData) {
        ...
        if (!zoneUid) {
          console.warn(
            "[ZONE_SELECT] tryOpenOverlay(sheet-zone) called with no uid/zone in context",
            contextData,
          );
        }
        const rates =
          contextData.rates && contextData.rates.length > 0
            ? contextData.rates
            : null;
        let duration = 0;
        const defaultDur = S.get.defaultDurationMinutes || 0;
        if (defaultDur > 0) {
          const zone = S.get.zones.find(
            (z) => z.uid === zoneUid || z.id === zoneUid,
          );
          const maxM =
        ...
        console.warn(
          "[ZONE_SELECT] tryOpenOverlay(sheet-zone) called with no context and no selectedZone",
        );
      }
    }
    ...
    if (
      (id === "modal-add-plate" || id === "modal-edit-plate") &&
      S.get.screen !== "plates"
    )
      return;
    ...
    if (
      ...
      Q8.UI &&
      Q8.UI.populateConfirmStartModal
    ) {
      const zoneObj =
        S.get.zones &&
        (S.get.zones.find((z) => z.uid === S.get.selectedZone) ||
          S.get.zones.find((z) => z.id === S.get.selectedZone));
      const zoneId = zoneObj ? zoneObj.id : S.get.selectedZone || "—";
      const adminPlates = (S.get.adminPlates || []).map((p) => ({
        id: p.id,
        text: p.text || p.id,
        default: false,
      }));
      ...
      const selPlate =
        allPlates.find((p) => p.id === S.get.selectedPlateId) ||
        allPlates.find((p) => p.default) ||
        allPlates[0];
      const plateText = selPlate ? selPlate.text || selPlate.id : "";
      const opts =
        contextData && contextData.daypassNote
          ? { daypassNote: contextData.daypassNote }
          : {};
    ...
    if (
      id === "sheet-zone" &&
      contextData &&
      (contextData.uid || contextData.zone)
    ) {
      if (Q8.UI && Q8.UI.centerMapOnZones)
        setTimeout(() => Q8.UI.centerMapOnZones(), 50);
    }
  ...
  function getDayPassCost(zone) {
    if (!zone) return null;
    const rates = zone.rates || [];
    const dagkaartRate = rates.find((r) =>
      /dagkaart|day.?pass/i.test(r.detail || ""),
    );
  ...
  function validateStartParking() {
    ...
    if (S.get.session) {
      return {
        valid: false,
        message: nl
          ? "Je hebt al een actieve parkeersessie."
    ...
    if (!S.get.selectedZone) {
      return {
        valid: false,
        message: nl
          ? "Selecteer eerst een parkeerzone."
    ...
    if (!zoneObj) {
      return {
        valid: false,
        message: nl
          ? "Zone niet meer beschikbaar. Selecteer de zone opnieuw."
          : "Zone no longer available. Please select the zone again.",
        field: "zone",
      };
    }
    const selPlate =
      S.get.plates.find((p) => p.id === S.get.selectedPlateId) ||
    ...
    if (!selPlate || !plateText) {
      return {
        valid: false,
        message: nl ? "Selecteer een kenteken." : "Select a license plate.",
        field: "plate",
      };
    }
    ...
    if (
      ...
      return {
        valid: false,
        message: nl
          ? "Parkeren is niet toegestaan op deze dag."
          : "Parking is not allowed on this day.",
        field: "zone",
      };
    }
    if (ds.allowedTimeStart || ds.allowedTimeEnd) {
      ...
      const startM = parseTime(ds.allowedTimeStart),
        endM = parseTime(ds.allowedTimeEnd);
      if (startM != null && mins < startM) {
        return {
          valid: false,
          message: nl
            ? "Parkeren nog niet toegestaan."
            : "Parking not yet allowed.",
          field: "zone",
        };
      }
      if (endM != null && mins > endM) {
        return {
          valid: false,
          message: nl
            ? "Parkeren niet meer toegestaan vandaag."
            : "Parking no longer allowed today.",
          field: "zone",
        };
      }
    }
  ...
  function handleStartParking(options) {
    const skipOverlayCheck =
      options &&
      (options.fromDayPassConfirm === true ||
        options.fromConfirmStart === true);
    if (S.get.session) {
    ...
    if (!skipOverlayCheck && S.get.activeOverlay !== "sheet-zone") {
      console.warn(
        "[PARKING_START] Blocked: overlay is not sheet-zone",
        S.get.activeOverlay,
      );
      toast(
        S.get.language === "nl"
    ...
    if (!zoneObj) {
      console.warn(
        "[PARKING_START] Zone not found in zones list",
        S.get.selectedZone,
        "zones count:",
        S.get.zones.length,
      );
    ...
    const endDate =
      S.get.duration === 0
        ? null
        : new Date(now.getTime() + S.get.duration * 60000);
    const session = {
      zone: displayId,
    ...
    try {
      localStorage.setItem(
        "q8_last_used_zone_uid",
        String(zoneObj.uid || zoneObj.id),
      );
    ...
    if (endDate && requestNotificationPermission)
  ...
  function addNotification(type, message, detail) {
    ...
    if (
      ...
      try {
        new Notification("Q8 Parking", {
          body: message,
          icon: "/icons/favicon-32x32.png",
          tag: "parking-expiring",
        });
      } catch (e) {
        /* ignore */
      }
    ...
    db.collection("users")
      ...
      .update({
        notificationSettings: {
          ...
          expiringSoonMinutes:
            typeof settings.expiringSoonMinutes === "number"
              ? settings.expiringSoonMinutes
              : 10,
        },
  ...
  function handleAutoEndSession(reason) {
    ...
    if (reason === "sessionEndedByUser") {
      addNotification(
        "sessionEndedByUser",
        S.get.language === "nl"
          ? "Parkeersessie automatisch beëindigd (eindtijd bereikt)"
          : "Parking session ended automatically (end time reached)",
        `${zoneLabel} · ${plate}`,
      );
    } else {
      addNotification(
        "sessionEndedByMaxTime",
        S.get.language === "nl"
          ? "Parkeersessie beëindigd (maximale parkeertijd bereikt)"
          : "Parking session ended (maximum parking time reached)",
        `${zoneLabel} · ${plate}`,
      );
    }
  }
  ...
  function handleEndParking() {
    ...
    addNotification(
      "sessionEndedByUser",
      S.get.language === "nl"
        ? "Parkeersessie beëindigd"
        : "Parking session ended",
      `${session.zone} · ${session.plate || ""}`,
    );
    const endMsg =
      S.get.language === "nl"
        ? "Parkeersessie beëindigd"
  ...
  function modifyActiveSessionEnd(delta) {
    ...
    if (!session.end) {
      ...
      const endDate =
        session.end instanceof Date ? session.end : new Date(session.end);
      if (delta > 0) {
    ...
    if (hitLimit && delta > 0) {
      ...
      if (maxDurMins >= 1440) {
        toast(
          nl ? "Maximale parkeerduur bereikt" : "Max parking duration reached",
        );
      } else {
        ...
        toast(
          nl
            ? `Maximale parkeerduur voor deze zone is ${hoursStr}`
            : `Maximum parking duration for this zone is ${hoursStr}`,
        );
      }
    }
  }
  ...
  function saveNewPlate() {
    ...
    if (!rawVal)
      return toast(
        nl ? "Voer een kenteken in" : "Please enter a license plate",
      );
    ...
    if (Kenteken) {
      ...
      if (normalized.length > 8)
        return toast(nl ? "Kenteken te lang" : "License plate too long");
      if (!/^[A-Z0-9]+$/.test(normalized))
        return toast(
          nl ? "Alleen letters en cijfers" : "Letters and digits only",
        );
    }
    ...
    if (!formatValid)
      return toast(
        formatError ||
          (nl ? "Ongeldig kentekenformaat" : "Invalid license plate format"),
      );
    if (S.get.plates.some((p) => p.text === normalized || p.id === normalized))
      return toast(nl ? "Kenteken bestaat al" : "License plate already exists");

    const isFirst = S.get.plates.length === 0;
    const displayText =
      Kenteken && Kenteken.formatDisplay
        ? Kenteken.formatDisplay(normalized)
        : normalized;
    const newPlates = [
      ...S.get.plates,
      {
    ...
    if (Kenteken && Kenteken.lookupRDW) {
      Kenteken.lookupRDW(normalized)
        .then(function (result) {
          if (result.found && result.data) {
            const brand =
              (result.data.merk || "") +
              (result.data.handelsbenaming
                ? " " + result.data.handelsbenaming
                : "");
            toast(
              S.get.language === "nl"
                ? "Kenteken toegevoegd (gecontroleerd: " +
                    (brand.trim() || "RDW") +
                    ")"
                : "License plate added (verified: " +
                    (brand.trim() || "RDW") +
                    ")",
            );
            ...
            toast(
              S.get.language === "nl"
                ? "Kenteken toegevoegd (controle RDW tijdelijk niet beschikbaar)"
                : "License plate added (RDW check temporarily unavailable)",
            );
          } else {
            toast(
              S.get.language === "nl"
                ? "Kenteken toegevoegd (niet in RDW gevonden)"
                : "License plate added (not found in RDW)",
            );
          }
        })
        .catch(function () {
      ...
      toast(
        S.get.language === "nl" ? "Kenteken toegevoegd" : "License plate added",
      );
    }
  }
  ...
  function deletePlate(id) {
    ...
    toast(
      S.get.language === "nl" ? "Kenteken verwijderd" : "License plate deleted",
    );
  }
  ...
  function updatePlate(id, newText, newDescription) {
    ...
    if (
      S.get.plates.some(
        (p) =>
          p.id != id &&
          p.text != id &&
          (p.text === normalized || p.id === normalized),
      )
    )
      return toast(nl ? "Kenteken bestaat al" : "License plate already exists");
    ...
    S.savePlates();
    toast(
      S.get.language === "nl" ? "Kenteken bijgewerkt" : "License plate updated",
    );
  }
  ...
  function checkPlateRDW() {
    ...
    if (!rawVal) {
      resultEl.textContent =
        S.get.language === "nl"
          ? "Voer eerst een kenteken in."
          : "Enter a license plate first.";
    ...
    resultEl.textContent =
      S.get.language === "nl" ? "Controleren bij RDW..." : "Checking RDW...";
    resultEl.classList.add("plate-rdw-loading");

    Kenteken.lookupRDW(v.normalized)
      .then(function (result) {
        ...
        if (result.found && result.data) {
          ...
          resultEl.textContent =
            (S.get.language === "nl" ? "Gevonden: " : "Found: ") +
            (brand || soort || "RDW");
          resultEl.classList.add("plate-rdw-ok");
        } else {
          resultEl.textContent =
            S.get.language === "nl"
              ? "Niet gevonden in RDW-register."
              : "Not found in RDW register.";
      ...
      .catch(function () {
        resultEl.classList.remove("plate-rdw-loading");
        resultEl.textContent =
          S.get.language === "nl" ? "Controle mislukt." : "Check failed.";
        resultEl.classList.add("plate-rdw-error");
      });
  }
  ...
  function modifyDuration(delta) {
    if (S.get.activeOverlay !== "sheet-zone") {
      console.warn(
        "[DURATION] modifyDuration ignored: overlay not sheet-zone",
        S.get.activeOverlay,
      );
      return;
    }
  ...
  function setDefaultPlate() {
    ...
    const targetPlate = S.get.plates.find(
      (p) => p.id === S.get.selectedPlateId,
    );
    if (!targetPlate) {
      console.warn(
        "[PLATES] setDefaultPlate: selected plate not in list",
        S.get.selectedPlateId,
      );
    }
  ...
  function checkInstallMode() {
    ...
    if (urlParams.has("forceInstall")) {
      ...
      S.update({
        installMode: {
          ...S.get.installMode,
          active: true,
          platform: isIOS ? "ios" : isAndroid ? "android" : "generic",
        },
      });
      return;
    }
    ...
    if (isTouch && !isStandalone) {
      ...
      try {
        visitCount =
          parseInt(localStorage.getItem("q8_install_visit_count") || "0", 10) +
          1;
