  ...
  function saveNewPlate() {
    ...
    if (Kenteken) {
      ...
      if (!/^[A-Z0-9]+$/.test(normalized))
        return toast(nl ? 'Alleen letters en cijfers' : 'Letters and digits only');
    }

    if (!formatValid)
      return toast(
        formatError || (nl ? 'Ongeldig kentekenformaat' : 'Invalid license plate format')
      );
    ...
    Kenteken.lookupRDW(v.normalized)
      .then(function (result) {
        ...
        if (result.found && result.data) {
          ...
          resultEl.textContent =
            (S.get.language === 'nl' ? 'Gevonden: ' : 'Found: ') + (brand || soort || 'RDW');
